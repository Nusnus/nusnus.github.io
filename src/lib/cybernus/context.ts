 ...
 *
 *   2. `## Current Spectrum Setting` — the Groky slider position + the
 *      explicit behavioural instruction for that band.
 *
...
function buildRuntimeEnvironment(p: CybernusContextParams): string {
  ...
  const langDesc =
    p.language === 'es'
      ? 'Spanish (Cali, Colombia style — warm, casual, "parce" and "bacano" welcome, zero stiff textbook Spanish). Default to Spanish unless the visitor writes in English.'
      : 'English. Default to English unless the visitor writes in Spanish.';
