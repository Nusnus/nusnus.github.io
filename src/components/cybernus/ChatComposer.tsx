 ...
 *
 * Textarea + send button + voice dictation button + stop button.
 * Auto-grows up to a cap. Enter sends, Shift+Enter inserts newline.
 *
...
export interface ChatComposerHandle {
  /** Replace the composer contents and focus it (suggested-question chips). */
...
interface ChatComposerProps {
  ...
  onStop: () => void;
  /** True while the assistant is streaming — swap send for stop. */
  streaming: boolean;
...
/** Lucide-style inline icons (keeps the bundle light — no icon library import). */
const IconSend = () => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
...
export const ChatComposer = memo(
  forwardRef<ChatComposerHandle, ChatComposerProps>(function ChatComposer(
    ...
    // Voice input — the hook fires `onFinal` when recognition ends, so we
    // consume the transcript imperatively (no state-sync effect needed).
    const speech = useSpeechInput(language, {
      onFinal: (text) => setValue((v) => (v ? v + ' ' : '') + text),
    });

    // Expose the seed method to the parent via ref.
    useImperativeHandle(
      ref,
      () => ({
        seed: (text: string) => {
    ...
    return (
      <form onSubmit={onSubmit} className="relative">
        <div className="border-border bg-bg-surface/80 focus-within:border-accent/50 flex items-end gap-2 rounded-2xl border p-2 backdrop-blur-sm transition-colors">
          ...
          {/* Mic button — only rendered if the browser supports Web Speech. */}
          {speech.supported && (
            <button
              type="button"
              onClick={toggleMic}
